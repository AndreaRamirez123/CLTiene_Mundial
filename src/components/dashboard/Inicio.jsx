  ...
  return (
    <div>
      ...
      <div style={{ background: "linear-gradient(135deg, #7c1a8c, #1a0f3d)", border: "1px solid rgba(130,43,210,0.5)", borderRadius: 16, padding: "18px 16px", marginBottom: 16, position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ color: "var(--texto-sec)", fontSize: 12, marginBottom: 8 }}>El torneo comienza en</div>
        <div style={{ display: "flex", gap: 10, marginBottom: 12 }}>
          {[["85", "DÍAS"], ["14", "HRS"], ["32", "MIN"]].map(([n, l]) => (
            <div key={l} style={{ background: "rgba(0,0,0,0.3)", borderRadius: 10, padding: "10px 14px", textAlign: "center", minWidth: 56 }}>
              <div style={{ color: "var(--texto)", fontSize: 24, fontWeight: 900, lineHeight: 1 }}>{n}</div>
              <div style={{ color: "var(--texto-ter)", fontSize: 10 }}>{l}</div>
            </div>
          ))}
        </div>
        <div style={{ color: "var(--texto)", fontWeight: 700, fontSize: 14 }}>🏟️ USA · México · Canadá 2026</div>
      </div>

      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        <span style={{ color: "var(--texto)", fontWeight: 800, fontSize: 16 }}>⚽ Próximos partidos</span>
      ...
      {proximos.length === 0 ? (
        <div style={{ color: "var(--texto-ter)", fontSize: 14, textAlign: "center", padding: "20px 0" }}>Cargando partidos...</div>
      ) : proximos.map((p, idx) => (
        <div key={p.id} className="anim-slide-up" style={{ background: "var(--card)", border: "1px solid var(--card-border)", borderRadius: 14, padding: "14px 16px", marginBottom: 10, animationDelay: `${idx * 0.1}s`, animationFillMode: "both" }}>
          ...
          <button onClick={() => setTab("polla")} style={{ width: "100%", marginTop: 12, padding: "9px", background: "linear-gradient(135deg, #FD7751, #e5622a)", border: "none", borderRadius: 10, color: "var(--texto)", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
            ⚽ Hacer predicción
          </button>
        </div>
      ))}
