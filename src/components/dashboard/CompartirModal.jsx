...
export default function CompartirModal({ onCompartir, onCerrar, redesSociales, claveReferido }) {
  return (
      ...
      <div style={{ background: "linear-gradient(135deg, var(--card) 0%, rgba(var(--card-rgb, 26, 17, 48), 0.95) 100%)", borderRadius: 20, padding: 28, maxWidth: 400, width: "100%", boxShadow: "0 20px 60px rgba(0,0,0,0.4), inset 0 1px 1px rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.1)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          <div>
            ...
            <div style={{ color: "var(--texto-sec)", fontSize: 12, marginTop: 2 }}>Gana monedas por cada registro</div>
          </div>
        ...
        <div style={{ background: "linear-gradient(135deg, rgba(236,168,45,0.15) 0%, rgba(236,168,45,0.05) 100%)", border: "1.5px solid rgba(236,168,45,0.3)", borderRadius: 14, padding: 16, marginBottom: 20, textAlign: "center" }}>
          <div style={{ color: "var(--texto-sec)", fontSize: 11, fontWeight: 600, letterSpacing: 1, marginBottom: 6 }}>TU NICK DE REFERIDO</div>
          <div style={{ color: "#ECA82D", fontWeight: 900, fontSize: 24, letterSpacing: 1, fontFamily: "monospace" }}>@{claveReferido || "tu-nick"}</div>
          <div style={{ color: "var(--texto-ter)", fontSize: 11, marginTop: 6 }}>Cuando lo usen, ambos ganan +50 🪙</div>
        </div>

        <div style={{ color: "var(--texto-sec)", fontSize: 13, marginBottom: 18, textAlign: "center", lineHeight: 1.5 }}>
          Comparte por tus redes sociales. Cuando alguien use tu nick, ambos ganan monedas.
        </div>

        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginBottom: 16 }}>
          {redesSociales.map((red) => {
            const Icono = ICONOS[red.icono] || ICONOS[red.id];
            return (
              <button
                key={red.id}
