    ...
    <div>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        {ranking.slice(0, 3).map((j, i) => {
          const medallas = ["🥇", "🥈", "🥉"];
          return (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, background: j.esYo ? "rgba(253,119,81,0.1)" : "var(--card)", border: j.esYo ? "2px solid #FD7751" : "1px solid var(--card-border)", borderRadius: 12, padding: "12px 14px", marginBottom: 8 }}>
              <span style={{ fontSize: 24 }}>{medallas[i]}</span>
              <div style={{ flex: 1 }}>
                <span style={{ color: j.esYo ? C.naranja : "var(--texto)", fontWeight: 700, fontSize: 14 }}>{j.nombre} {j.esYo ? "👈" : ""}</span>
              </div>
