...
export default function Dashboard({ usuario, onCerrarSesion }) {
  ...
  const reclamarBono = async () => {
    try {
      ...
      alert("Ya reclamaste tu bono hoy o hubo un error");
    }
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0f0a1e", fontFamily: "'Segoe UI', sans-serif", paddingBottom: 80 }}>
      ...
      <div style={{ background: "rgba(15,10,30,0.95)", borderBottom: "2px solid #FD7751", padding: "10px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 100 }}>
        ...
        <button onClick={cerrar} style={{ background: "transparent", border: "1px solid rgba(255,255,255,0.15)", color: C.gris, borderRadius: 8, padding: "5px 12px", cursor: "pointer", fontSize: 13 }}>
          Salir
        </button>
      </div>
        ...
        <div style={{ position: "relative", maxWidth: 600, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
            {[
              ...
              { label: "Posición", valor: "#4", icono: "🏆", color: C.naranja },
            ].map((s, i) => (
      ...
      <div style={{ maxWidth: 600, margin: "0 auto", padding: "20px 14px" }}>
        {tab === "inicio" && <Inicio setTab={setTab} reclamarBono={reclamarBono} />}
        {tab === "polla" && <Polla monedas={monedas} usuario={usuario} cargarPerfil={cargarPerfil} />}
        {tab === "ranking" && <Ranking />}
...
function Inicio({ setTab, reclamarBono }) {
  return (
    <div>
      {/* Cuenta regresiva */}
      ...
      {PARTIDOS.slice(0, 3).map((p) => (
        <div key={p.id} style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, padding: "14px 16px", marginBottom: 10 }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
            <span style={{ background: "rgba(253,119,81,0.2)", color: "#FD7751", fontSize: 11, fontWeight: 700, padding: "2px 8px", borderRadius: 20 }}>{p.fase}</span>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr auto 1fr", alignItems: "center", gap: 8 }}>
            <div style={{ textAlign: "center" }}>
              ...
              <div style={{ color: "#fff", fontWeight: 700, fontSize: 13, marginTop: 4 }}>{p.local}</div>
            </div>
            ...
            <div style={{ textAlign: "center" }}>
              ...
              <div style={{ color: "#fff", fontWeight: 700, fontSize: 13, marginTop: 4 }}>{p.visitante}</div>
            </div>
          </div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", margin: "20px 0 12px" }}>
        <span style={{ color: "#fff", fontWeight: 800, fontSize: 16 }}>🏆 Top del ranking</span>
      </div>
      {RANKING.slice(0, 3).map((j) => (
        <div key={j.pos} style={{ display: "flex", alignItems: "center", gap: 12, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 10, padding: "10px 14px", marginBottom: 8 }}>
          <span style={{ color: "#ECA82D", fontWeight: 800, fontSize: 14, minWidth: 24 }}>#{j.pos}</span>
          <span style={{ flex: 1, color: "#fff", fontSize: 14 }}>{j.nombre}</span>
          <span style={{ color: "#ECA82D", fontSize: 13, fontWeight: 700 }}>🪙 {j.monedas}</span>
        </div>
      ))}
    </div>
  );
...
function Polla({ monedas, usuario, cargarPerfil }) {
      ...
      {PARTIDOS.map((p) => (
        <div key={p.id} style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 16, padding: "16px", marginBottom: 14 }}>
          ...
          <button
            ...
            style={{ width: "100%", padding: "12px", background: enviando[p.id] ? "rgba(253,119,81,0.5)" : "linear-gradient(135deg, #FD7751, #e5622a)", border: "none", borderRadius: 12, color: "#fff", fontWeight: 800, fontSize: 15, cursor: enviando[p.id] ? "not-allowed" : "pointer" }}>
  ...
  return (
    <div>
      <div style={{ background: "linear-gradient(135deg, rgba(236,168,45,0.15), rgba(253,119,81,0.1))", border: "1px solid rgba(236,168,45,0.3)", borderRadius: 14, padding: "14px 16px", marginBottom: 16, textAlign: "center" }}>
        ...
        <div style={{ color: "#ECA82D", fontSize: 36, fontWeight: 900 }}>#4</div>
        <div style={{ color: "rgba(255,255,255,0.4)", fontSize: 12 }}>¡Sigue prediciendo para subir!</div>
      </div>
      {RANKING.map((j, i) => (
        <div key={j.pos} style={{ display: "flex", alignItems: "center", gap: 14, background: j.esYo ? "rgba(253,119,81,0.1)" : "rgba(255,255,255,0.04)", border: j.esYo ? "2px solid #FD7751" : "1px solid rgba(255,255,255,0.07)", borderRadius: 14, padding: "14px 16px", marginBottom: 10 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ color: "rgba(255,255,255,0.35)", fontSize: 12, marginTop: 2 }}>{j.aciertos} predicciones acertadas</div>
          </div>
